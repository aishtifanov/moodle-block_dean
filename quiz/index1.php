<?php // $Id: index.php,v 1.0 2011/03/28 16:00:00 zagorodnyuk Exp $
    ...
    require_login();
    $link=mysql_connect('bsu-dekanat.bsu.edu.ru','dekanadm','big#psKT');
			...
			if($error == 0) {
              ...
              //  $auditories = get_records_select('bsu_schedule', "edworkid=$sql->edworkid AND groupno='$sql->groupno'", '', 'id, roomid, datestart, timestart, timeend');
				...
				//	foreach($auditories as $auditorie) {
						...
						$data_2 = date("Y-m-d",$auditorie['datestart']);
						$timeopen = 0;
						$timeclose = 0;

						if(count($data_1) == 2) {
							$data_t = explode('-', $data_1[0]);
							$timeopen = mktime(date("H",$auditorie['timestart'])-1, date("i",$auditorie['timestart']), 0, $data_t[1], $data_t[2], $data_t[0]);
							$data_t = explode('-', $data_2[0]);
							$timeclose = mktime(date("H",$auditorie['timeend'])-1, date("i",$auditorie['timeend']), 0, $data_t[1], $data_t[2], $data_t[0]);
						}
	...
	if($id == 0) {
		if($action == 'alld' && ($dean_is || $admin_is)) {
			...
			listbox_subdepartment($scriptname, $subid, $dean_is);
			echo '</table></center>';
		...
		if($courseids) {
			...
			$ids = implode(',', $ids);
	        $courses = get_records_select('course', "id in ($ids) AND category=64", '', 'id, fullname');
	        if($courses) {
