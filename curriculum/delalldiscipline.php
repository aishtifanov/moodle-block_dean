<?PHP // $Id: delalldiscipline.php,v 1.1.1.1 2009/08/21 08:38:45 Shtifanov Exp $
	...
	if (isset($confirm)) {	    $strsql = 'DELETE FROM '. $CFG->prefix . 'dean_discipline WHERE curriculumid=' . $cid . ';';

	    if (!($rs = $db->Execute($strsql))) {
				error(get_string('errorindeletedalldiscipline','block_dean'), "$CFG->wwwroot/blocks/dean/curriculum/disciplines.php?fid=$fid&amp;sid=$sid&amp;cid=$cid");
		}
		// delete_records('dean_discipline', 'id', $did);
		add_to_log(1, 'dean', 'All discipline deleted in curriculumid' . $cid, 'delalldiscipline.php', $USER->lastname.' '.$USER->firstname);
	...
	print_heading(get_string('deletealldiscipline','block_dean'));

    $strall = get_string('alldiscipline', 'block_dean');
	notice_yesno(get_string('deletecheckfull', '', $strall),
               "delalldiscipline.php?fid=$fid&amp;sid=$sid&amp;cid=$cid&amp;confirm=1", "disciplines.php?fid=$fid&amp;sid=$sid&amp;cid=$cid");
