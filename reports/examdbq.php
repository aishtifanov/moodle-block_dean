<?php // $Id: examdbq.php,v 1.4 2011/06/24 07:10:32 shtifanov Exp $
    ...
    if ($action == 'excel') 	{
        ...
        if ($timestartday == 1 || $timestartday == 2) {
            ignore_user_abort(false); 
            @set_time_limit(0);
            @ob_implicit_flush(true);
            @ob_end_flush();
	        @raise_memory_limit("258M");
...
function table_examreportattempts($timestartday, $notify = true)	
{
	...
	$countrows = 0;
	if ($timestartday == 1)	{
	    $timestartday = make_timestamp(2011, 04, 11);
		$timeendday = time();
	} else if ($timestartday == 2)	{
	    $timestartday = make_timestamp(2010, 12, 20);
        $timeendday  = make_timestamp(2011, 04, 11);
	} else {	
