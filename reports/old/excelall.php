<?php // $Id: excelall.php,v 1.2 2011/03/22 06:56:09 shtifanov Exp $
...
/// Print names of all the fields
        ...
        $myxls->write_string(1,0, get_string('group','block_dean'),$formath2);
        ...
        $myxls->write_string(1,4,'Число студентов, имеющих задолжность',$formath2);

////////////////////

	 $sdal = $otlichniki =  $dolg = $kach = $kolstudents = 0;
      ...
      $g = 0;
	     if($currriculums = get_records('dean_curriculum','facultyid',$fid))  {
            ...
            $kach = round ($otlichniki/$kolstudents*100.0);
             $g++;
              $myxls->write_string($g,0,$groups->name,$formatp);
          	  $myxls->write_string($g,1,$otlichniki,$formatp);
    	   	  $myxls->write_string($g,2,$kach,$formatp);
           	  $myxls->write_string($g,3,$sdal,$formatp);
           	  $myxls->write_string($g,4,$dolg,$formatp);
	}
