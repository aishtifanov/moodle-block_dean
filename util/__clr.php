<?php // $Id: __clr.php,v 1.1.1.1 2009/10/29 08:23:05 Shtifanov Exp $
	...
	$breadcrumbs .= " -> __Clear all LDAP-users";
    print_header("$SITE->shortname: __CLear all LDAP-users", $SITE->fullname, $breadcrumbs);
...
{
	// $users = get_records('user', 'auth', 'ldap', 'id', 'id, auth');	
	$users = get_records('user', 'deleted', 1, 'id', 'id, deleted');
//	print_r($users); exit();    
    if ($users) {
        ...
        // continues
        // print($i . ' the LDAP users had been successfully removed.');
        print($i . ' the DELETED users had been successfully removed.');
        print_continue('__clr.php');
    } else {
        // no deletion
        print('None of the users had been removed.');
    }
}
