<?php // $Id: ldapsync__.php,v 1.10 2011/10/11 07:14:42 shtifanov Exp $
    ...
    if (isset($namegroup) && !empty($namegroup)) 	{
		...
		if ($agroup)	{
			...
			if ($astudents = get_records_sql($studentsql))	{
				...
				if ($lstudents = get_records_select('bsu_students', "grup= '$namegroup' and Dateotsh = ''", 'name', 'CodePhysPerson as login, Name, grup'))	{
					$ldapstuds = array();
					...
					foreach($lstudents as $lstudent)	{
						...
						$newusers[$lstudent->login]->Name =  trim($lstudent->Name);						
						$newusers[$lstudent->login]->grup =  trim($lstudent->grup);
						...
						$newusers[$lstudent->login]->group1 =  trim($lstudent->grup);
					...
					foreach ($deanstuds as $username => $deanstud) {
						...
						if ($deanstudsauth[$username] == 'cas')	{
							$studentLDAP = get_record('bsu_students', 'CodePhysPerson', $username);	
						} else {
							$studentDEAN = get_record_select('user', "id = {$deanstudsid[$username]}", 'id, lastname, firstname');
							// list ($afirstname, $asecondname) = explode(' ', $studentDEAN->firstname);
                            $fullname =  $studentDEAN->lastname . ' ' . $studentDEAN->firstname;
							$studentLDAP = get_record('bsu_students', 'name', $fullname);
						}
						...
						if 	($login === false)	 {
							...
							if ($username != $login)	{
				                ...
				                $rokirovkalink .= "<a title=\"$title\" href=\"ldapsync__.php?username=$username&amp;login=$login&amp;action=rokirovka&amp;namegroup=$namegroup&amp;go=1\">";
	...
	print_heading($strsynchronization . '(с таблицей хранилища bsu_students)', 'center', 2);
	// notify (get_string('description_synchronization','block_dean'), 'black');
	
    print_dean_box_start('center', '80%');


	echo '<div align=center><form name="studentform" id="studentform" method="post" action="ldapsync__.php">'.
	...
	if (isset($namegroup) && !empty($namegroup)) 	{	 
			...
			<form name="studentform" id="studentform" method="post" action="ldapsync__.php">'.
