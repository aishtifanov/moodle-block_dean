<?php // $Id: synchr_fid.php,v 1.1.1.1 2009/08/21 08:38:46 Shtifanov Exp $
			   ...
			   if ($tbFacultet  = xfgetcsv ($fp, 1000, $csv_delimiter))  {
         			if ($faculty = get_record('dean_faculty', 'number', $tbFacultet[3])) {
						...
						if ($dname != $tbFacultet[1])  {
						    $msg = get_string('newdean', 'block_dean').' '.$tbFacultet[0].':';
							notify($msg);
							notify($tbFacultet[1]);
						}
						if ($tbFacultet[0] != $faculty->name)   {
							$faculty->name = $tbFacultet[0];
							$faculty->timemodified = time();
							if (update_record('dean_faculty', $faculty))	{
								 notify(get_string('facultyupdate','block_dean').':'.$faculty->name);
							} else {
...
/// Print the form
    ...
    echo '<center>';
    echo '<form method="post" enctype="multipart/form-data" action="synchr_fid.php">'.
