<?php // $Id: s2.php,v 1.2 2012/10/19 06:24:42 shtifanov Exp $
    ...
    $strimportfac = get_string('s2','block_dean');
	...
	if ($fid != 0)  {
	    ...
	    if ($um->preprocess_files())  {
		    ...
		    while (!feof ($fp)) {
				...
				$username0 = trim($line[5]);
                if (!isset($username0) || empty($username0)) break;

	      		// print_r($line);  echo '<hr>';

                if ($olduser = get_record_sql("SELECT id, auth, username, firstname, lastname
                						FROM {$CFG->prefix}user
                						WHERE username = '$username' AND auth = 'cas'"))	{

					if ($olduser->lastname == $lastname)  {
					    $olduser->username = $username0;
						if(update_record("user", $olduser))  {
							 notify("Update $struser: $username ==> $olduser->username ", 'green', 'left');
		                     $numusers++;
  						}
	                } else {
		                notify("$struser: $olduser->username  <> $lastname");
		                $errornum++;
		            }

                } else {
					    fwrite($fnot, "$username;$lastname;$firstname;$groupnumber;$username0;\n");
					    /*
						if ($academygroup = get_record('dean_academygroups', 'name', $groupnumber))		{
							...
							if (!$flagexistence) {
								if($user->id = insert_record("user", $user))  {
									 notify("INSERT $struser: $user->username ", 'green', 'left');
				                     $numusers++;
		  						}
								else {
								    ...
								    fwrite($fexist, "$username;$lastname;$firstname;$groupnumber;$username0;\n");
...
/// Print the form
    ...
    echo '<center>';
    echo '<form method="post" enctype="multipart/form-data" action="s2.php">'.
