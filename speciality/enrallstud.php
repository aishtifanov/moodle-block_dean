<?php // $Id: enrallstud.php,v 1.6 2013/09/03 14:01:08 shtifanov Exp $
    ...
    $strimportfac = get_string('enrolmentnew') . ' ТОЗ';
...
//	listbox_courses_TOZ("enrallstud.php?fid=$fid&amp;did=", $courseid);
//	echo '</table>';


	echo '<table cellspacing="0" cellpadding="10" align="center" class="generaltable generalbox">';
	echo '<form name="chooseparam" method="post" action="enrallstud.php">';
	echo '<tr><td>'.get_string('discipline', 'block_dean').':</td><td>';
    echo combobox_courses_TOZ("enrallstud.php?fid=$fid&amp;did=", $courseid);    
	echo '</td></tr>';
    
	echo '<tr><td>'.get_string('courses').':</td><td>';
    for ($i=1; $i<=6; $i++) {
        $name = 'kurs'.$i;
        echo "<input name=$name type=checkbox value=1>$i-й курс<br>";    
    }
	echo '</td></tr>';
	echo '<tr><td align=center colspan=2>';    
	echo '<input type="hidden" name="fid" value="' . $fid . '">';
	echo '<input type="submit" name="setparam" value="' . get_string('go') . '">';
